...
1. **Governing track: prototype** — a customer-validation slice of the conversational intake experience on the archetype; the contracting engine (PA + Nexus contracts, signatures, CRM, fee-waiver) and the future AI/portal products are promotion-track and deferred. _Why: the differentiator is legally gated (see #2), and the intake slice is the only piece that can move now._ The architect treats this as settled.
...
10. **Designed for the post-hurricane, no-power moment** — the founder's stated biggest opportunity. Minimal typing, forgiving parse, correctable at Review, tolerant of a bad connection. This is _why_ the loop is utterance-first; it is a design constraint, not a preference.
...
Technical calls made while planning the build (see `mission/architecture.md`). Scope and track unchanged — these are _how_ to build the settled spec, not _what_ to build.
...
A5. **The raw utterance is not persisted** — only the extracted structured facts are saved. _Why:_ `scope.md` says the app will not record or store conversations; persisting the text would blur a line the distiller drew deliberately.
...
A8. **Welcome and the utterance box are merged onto `/`** — one route delivers loop steps 1–2; both [AC-1] and [AC-2] hold. _Why:_ a splash screen is a wasted tap for someone post-storm. Four routes total, within the ≤5-screen guideline.
...
_No Q&A round was needed — all technical ambiguity resolved against archetype defaults and the settled spec._
...
than overwriting the Homeowner's fields on submit. _Why: a demo claim stays
self-contained and readable without joins, and repeated demo runs with
different names can't cross-contaminate the one seeded homeowner._
...
submits, `/done` reads the stored claim back. _Why: a reload of the
confirmation — likely on a flaky connection — must never file a second demo
claim._ Server-side re-validation is unchanged (AC-8).
...
locally inert. _Trade-off accepted:_ `/tmp` is per-instance, so demo data
...
cookie small. What A6 was _for_ is preserved: the draft still survives
...
back.** _This rule is a founder decision, not spec-derived_ — the spec pack
...
_Pending founder call:_ the brief's landing H1 copy ("We're here to help")
...
prose after the city is never swallowed. _Known limit, accepted:_ a
...
pass for fuzzy fields — founder direction; amends A2.** A2's _spirit_ is
...
## Homepage copy (2026-08-01)

B22. **Sentence-length guidance lives in the subhead only.** The subhead now
begins “In your own words, a sentence is plenty.” (comma, no em dash), and
the duplicate “One sentence is enough” helper was removed from the input
card. AC-1 and its browser test were amended in the same change so this
founder-approved wording is executable product behavior, not an unpinned
visual tweak.

## Homepage marketing and staff access (2026-08-13)

B23. **Homepage promise narrowed to the behavior the current product performs.**
The opening subhead now promises natural-language intake, review, and team
follow-up—not automated filing, representation, inspection handling, repair
performance, or a particular outcome. Below the input, a reassurance strip,
three-step process, damage examples, FAQ, and Nexus Development attribution
make the current service boundary explicit. Public-adjuster participation,
the proposed conditional $25 fee, contract-package language, settlement
comparisons, check-payee strategy, Florida availability, and roofing-license
claims remain excluded from public copy pending counsel-approved exact
language and verified operating facts.

B24. **Internal access has two levels: `standard` and `admin`.** New staff
default to `standard`; the earlier speculative reviewer/manager vocabulary is
retired. Melissa and the business owner are the initial internal users, but
no role assignment or detailed permission matrix is inferred from that fact.
